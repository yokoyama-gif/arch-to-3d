"""Add images to ALL judgment sheets that don't have them yet."""
import openpyxl
...
total_added = 0; total_missing = 0

for sheet_name in JUDGMENT_SHEETS:
    ws = wb[sheet_name]
    # Skip sheets that already have images
    if len(ws._images) > 0:
        existing = len(ws._images)
        print(f'  {sheet_name}: SKIP (already has {existing} images)')
        continue
...
print(f'Saving... (this may take a moment with ~1000 images)')
wb.save(XLSX)
print(f'Done! +{total_added} images added, {total_missing} missing')
